...
A site's manifest. Put it in the site root; `devkit init [path]` makes the machine match it and
...
php: "8.4"                  # devkit isolate php@8.4 (must be installed)
...
mail: true                  # mailpit instance + MAIL_* + MAIL_FROM_ADDRESS + DEVKIT_MAIL_TAG + the client package
...
6. The client package via a Composer path repository (`~/Code/devkit/packages/devkit-client`).
7. `.env`: keys replaced in place (Laravel reads the first definition), new keys appended under `# devkit`, file created from `.env.example` if missing. `APP_URL` and `APP_NAME` are always set.
...
`redis`, `meilisearch`, `typesense`, `seaweedfs` (S3), `reverb` (per site — `--site` in the CLI, not via dev.yml),
`mailpit`, `dumps` (devkit's own). `devkit services:available` lists them with installed formulae.
