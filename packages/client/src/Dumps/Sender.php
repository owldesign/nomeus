...
/**
 * Sends structured events to nomeus's dump server using VarDumper's own wire format
 * (base64(serialize([Data, context])) per line), with a `nomeus` context naming the kind.
...
final class Sender
{
    ...
    {
        return $_SERVER['NOMEUS_REQUEST_ID'] ?? null;
    }
    ...
    {
        $nomeus = ['kind' => $kind, 'request_id' => self::requestId()] + $context;
        if (isset($_SERVER['REQUEST_METHOD'])) {   // a web request (fpm); the CLI has argv instead
            $nomeus += ['uri' => $_SERVER['REQUEST_URI'] ?? null, 'method' => $_SERVER['REQUEST_METHOD']];
        } else {
            $nomeus += ['command' => implode(' ', $_SERVER['argv'] ?? [])];
        }
        $data = (new VarCloner)->cloneVar($payload);

        return base64_encode(serialize([$data, ['nomeus' => $nomeus]]))."\n";
    }
