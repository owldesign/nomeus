...
/**
 * bin/devkit must hand every Valet command straight to valet. `use` was missing once and
 * `devkit use php@8.4` booted artisan instead — on a PHP artisan couldn't run on.
 */
function shimPassthrough(): array
{
    $shim = file_get_contents(base_path('bin/devkit'));
...
it('never lists a devkit command as valet passthrough', function () {
    $devkit = ['status', 'sites', 'site-information', 'tasks', 'php:list', 'php:install', 'php:update', 'ini', 'db', 'edit', 'config:get', 'config:set', 'services:create', 'services:list', 'services:upgrade', 'services:adopt', 'mail', 'init', 'logs', 'dumps', 'dumps:install', 'xdebug', 'xdebug:mode', 'doctor', 'self-update'];
