...
## Generated by `nomeus new`

`nomeus new shop --laravel --db=postgresql --redis --mail --secure` writes a manifest like the example above and runs
`init` on it. It picks instances you already have (`instance: pg17`), names the database after the site (`shop`), and
adds `CACHE_STORE`/`QUEUE_CONNECTION`/`SESSION_DRIVER=redis` when redis is chosen. `--dry-run` prints the file and the
init plan without creating anything; the same form exists on the Sites page.
