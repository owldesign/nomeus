...
beforeEach(function () {
    $this->dir = sys_get_temp_dir().'/nomeus-launchd-'.uniqid();
    mkdir($this->dir, 0755, true);
    $this->m = new LaunchdManager(new Shell(new NomeusConfig("{$this->dir}/cfg.json")), "{$this->dir}/agents", 501);
});
...
it('writes a well-formed, escaped plist', function () {
    ...
    expect($path)->toBe("{$this->dir}/agents/dev.nomeus.svc.pg.plist")
        ->and($xml)->toContain('<string>dev.nomeus.svc.pg</string>')
...
it('parses launchctl print into loaded/pid/state and reads the disabled list', function () {
    Process::fake([
        '*launchctl*print-disabled*' => Process::result("disabled services = {\n\t\"dev.nomeus.svc.pg\" => disabled\n\t\"com.apple.x\" => enabled\n}\n"),
        '*launchctl*print*' => Process::result("gui/501/dev.nomeus.svc.pg = {\n\tactive count = 1\n\tpath = /x\n\tstate = running\n\n\tpid = 4242\n\tlast exit code = 1\n}\n"),
    ]);
...
it('bootstraps only when not loaded, tolerates bootout of an unloaded job, and surfaces real failures', function () {
    ...
    $this->m->bootstrap('pg');
    Process::assertRan(fn ($p) => $p->command === ['launchctl', 'bootstrap', 'gui/501', "{$this->dir}/agents/dev.nomeus.svc.pg.plist"]);
